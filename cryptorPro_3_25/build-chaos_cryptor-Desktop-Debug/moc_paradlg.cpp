/****************************************************************************
** Meta object code from reading C++ file 'paradlg.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../chaos_cryptor/paradlg.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'paradlg.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_paraDlg[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       9,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x0a,
      36,    8,    8,    8, 0x0a,
      69,    8,   62,    8, 0x0a,
      82,    8,   62,    8, 0x0a,
      95,    8,   62,    8, 0x0a,
     108,    8,   62,    8, 0x0a,
     121,    8,    8,    8, 0x0a,
     140,    8,    8,    8, 0x0a,
     174,  172,  167,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_paraDlg[] = {
    "paraDlg\0\0on_confirmButton_clicked()\0"
    "on_cancelButton_clicked()\0double\0"
    "sendX0Slot()\0sendY0Slot()\0sendZ0Slot()\0"
    "sendW0Slot()\0showKeyboardSlot()\0"
    "confirmstringSlot(QString)\0bool\0,\0"
    "eventFilter(QObject*,QEvent*)\0"
};

void paraDlg::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        paraDlg *_t = static_cast<paraDlg *>(_o);
        switch (_id) {
        case 0: _t->on_confirmButton_clicked(); break;
        case 1: _t->on_cancelButton_clicked(); break;
        case 2: { double _r = _t->sendX0Slot();
            if (_a[0]) *reinterpret_cast< double*>(_a[0]) = _r; }  break;
        case 3: { double _r = _t->sendY0Slot();
            if (_a[0]) *reinterpret_cast< double*>(_a[0]) = _r; }  break;
        case 4: { double _r = _t->sendZ0Slot();
            if (_a[0]) *reinterpret_cast< double*>(_a[0]) = _r; }  break;
        case 5: { double _r = _t->sendW0Slot();
            if (_a[0]) *reinterpret_cast< double*>(_a[0]) = _r; }  break;
        case 6: _t->showKeyboardSlot(); break;
        case 7: _t->confirmstringSlot((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 8: { bool _r = _t->eventFilter((*reinterpret_cast< QObject*(*)>(_a[1])),(*reinterpret_cast< QEvent*(*)>(_a[2])));
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = _r; }  break;
        default: ;
        }
    }
}

const QMetaObjectExtraData paraDlg::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject paraDlg::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_paraDlg,
      qt_meta_data_paraDlg, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &paraDlg::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *paraDlg::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *paraDlg::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_paraDlg))
        return static_cast<void*>(const_cast< paraDlg*>(this));
    return QDialog::qt_metacast(_clname);
}

int paraDlg::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 9)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 9;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
